...
(function () {
  ...
  function splitStudents(model, students, by) {
    ...
    var out = [];
    var seen = {};
    function bucket(label, student) {
      var k = normKey(label);
      if (!seen[k]) { seen[k] = { label: label, students: [] }; out.push(seen[k]); }
    ...
    if (by === 'group' || by === 'teacher') {
      ...
      (model.groups || []).forEach(function (g) {
        ...
        if (by === 'group') {
          members.forEach(function (s) { bucket(g.name || g.code, s); });
        } else {
  ...
  function exportWorkbook(model, sheets, opts) {
    ...
    var placed = sheets.map(function (sheet) {
      return { name: exportSheetName(sheet.label, taken), label: sheet.label, students: sheet.students };
    });
    if (opts.summary !== false) {
      var rows = [['Namelist export'], ['Made', opts.madeAt || ''], ['Split by', opts.splitLabel || ''],
        ['Filters', opts.filterNote || 'none'], [],
        ['Sheet', 'Named for', 'Students']];
      placed.forEach(function (p) { rows.push([p.name, p.label, p.students.length]); });
      rows.push([]);
      rows.push(['Total rows', '', placed.reduce(function (n, p) { return n + p.students.length; }, 0)]);
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(rows),
        exportSheetName('Summary', taken));
    }
    placed.forEach(function (p) {
      var aoa = [exportHeader(columns)];
      p.students.forEach(function (s) {
        aoa.push(columns.map(function (key) { return exportCellValue(s, key); }));
      });
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(aoa), p.name);
    });
    return wb;
  }
