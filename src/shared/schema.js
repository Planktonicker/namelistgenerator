...
(function () {
    ...
    var data = {
      ...
      requests: (model.requests || []).map(function (r) {
        return { id: r.id, teacher: r.teacher, group: r.group, action: r.action,
          ...
          // when it was settled, so the teacher page can tell an answer to
          // this suggestion from an answer to an earlier one with the same id
          decided: r.decided || '' };
      }),
    };
  ...
  /* The band a student is really at in one column: the band written in the
   * cell, or their posting group's when the cell leaves it out. '' when the
   * column is empty, or when neither the cell nor the PG says a band — unknown
   * must not be read as high or as low. */
  function allocationBand(student, key) {
    var raw = norm(student.subjects && student.subjects[key]);
    if (!raw) return '';
    var p = sbbParts(raw);
    if (!p) {
      // A school-coded cell states its band outright: "English Language - G2 - K200".
      var coded = /\bG\s?([1-3])\b/i.exec(raw);
      return coded ? 'G' + coded[1] : '';
    }
    return p.band || pgBand(student.pg);
  }

  function bandAbove(student, key, cap) {
    var band = allocationBand(student, key);
    if (!band) return false;
    return Number(band.slice(1)) > Number(norm(cap).slice(1));
  }
  ...
  function matchers(group) {
    ...
    function add(key, value, without) {
      ...
      /* "!HIST>G1" — not taken ABOVE that band. Blank counts as not above, so
       * a class can be "everyone the higher classes did not take". */
      var notAbove = '';
      var cap = /^(.*?)\s*>\s*G?([1-3])$/i.exec(key);
      if (cap) { key = norm(cap[1]); notAbove = 'G' + cap[2]; without = false; }
      if (!key || seen[normKey(key)]) return;
      seen[normKey(key)] = true;
      var values = (without || notAbove) ? [] : norm(value).split('|').map(norm).filter(Boolean);
      out.push({
        key: key, values: values, value: values[0] || '',
        without: !!without, notAbove: notAbove,
      });
    }
  ...
  function matchesRule(student, group) {
    ...
    return matchers(group).every(function (m) {
      ...
      if (m.notAbove) return !bandAbove(student, m.key, m.notAbove);
  ...
  function groupSubjectKeys(group, subjectKeys) {
    var keys = matchers(group).filter(function (m) { return !m.without && !m.notAbove; })
  ...
  /* The subjects the school actually teaches, named the way the classes name
   * them — the same list the editor's classes table filters by, so a subject
   * is not called "HUM" on one page and "Social Studies" on the other.
   *
   * Each carries the workbook column(s) its classes are built on, because
   * filtering students by a subject means looking in a column, and one subject
   * can sit on more than one: "Social Studies" is on both SS and HUM if that
   * is how the classes were made.
   *
   *   [{ label, keys: [...], codes: [...] }]
   */
  function subjectChoices(model) {
    var seen = {};
    var out = [];
    (model.groups || []).forEach(function (g) {
      var label = norm(g.subject);
      if (!label) return;
      var k = normKey(label);
      if (!seen[k]) { seen[k] = { label: label, keys: [], codes: [] }; out.push(seen[k]); }
      seen[k].codes.push(g.code);
      groupSubjectKeys(g, model.subjectKeys).forEach(function (key) {
        if (seen[k].keys.indexOf(key) === -1) seen[k].keys.push(key);
      });
    });
    out.sort(function (a, b) { return cmp(a.label, b.label); });
    return out;
  }

  /* The other direction: what a single column is called. "HIST" the heading,
   * "History" the word the classes on it use — the commonest wins, so one
   * oddly named class does not rename a subject for everybody. */
  ...
  function subjectChoiceFor(model, label) {
    var want = normKey(label);
    return subjectChoices(model).filter(function (c) { return normKey(c.label) === want; })[0] || null;
  }
  ...
  function namelistHtml(group, members, esc, opts) {
    ...
    /* A non-breaking space in the name cell, rather than a height in
     * millimetres: one line of the same text in the same padding is the same
     * height as a row with a name in it, whatever the row height turns out to
     * be — including when a long class has its rows tightened. */
  ...
  /* Roomiest first. Each step takes a little air out of the rows — the space
   * above and below, then the leading — without touching the size of the type
   * or of the boxes, which is what a scale would do. */
  var ROW_STEPS = [
    { pad: 5, lh: 1.5 },
    { pad: 4, lh: 1.45 },
    { pad: 3, lh: 1.4 },
  ...
  function fitPrintSheet(sheet) {
    ...
    Array.prototype.forEach.call(sheet.querySelectorAll('.card'), function (card) {
      card.style.zoom = '';
      card.style.removeProperty('--nl-rowpad');
      card.style.removeProperty('--nl-rowlh');
      var h = card.getBoundingClientRect().height;
      /* Air between the rows is the cheapest thing to give up — nobody reads a
       * namelist and misses it — so it goes one step at a time, and no further
       * than it has to. Forty names reach one sheet this way with the type
       * still at full size. */
      for (var i = 1; i < ROW_STEPS.length && h > page; i++) {
        card.style.setProperty('--nl-rowpad', ROW_STEPS[i].pad + 'px');
        card.style.setProperty('--nl-rowlh', String(ROW_STEPS[i].lh));
   ...
   * The editor copies namelist.xlsx into backups\ before every save it makes,
   * which is the right instinct and a folder that grows forever. Two kinds of
   * copy are worth keeping, and they answer different questions:
   *
   *   the newest few   "undo what I did five minutes ago"
   *   one a day        "put back what we had before Monday's import"
   *
   * A backup is named from the modified time of the file it copied, so
   * namelist-20260818-1632.xlsx IS the workbook as it stood at 16:32 on the
   * 18th. The earliest name carrying a date is therefore the state that day
   * opened with — which is why the daily keeper is the FIRST of each day and
   * not the last.
   *
   * The names are zero-padded and big-endian, so this needs no file metadata
   * at all: sorting the names is sorting by time. (stamp() is local time, so
   * the hour that repeats at a daylight-saving fall-back can sort an hour out
   * of true once a year. The worst that costs is a keeper an hour off.)
   *
   * "The last 14 days" means the fourteen most recent dates that actually
   * appear, not a fortnight of calendar: somebody who opens the editor once a
   * week still gets fourteen rewind points rather than one.
   *
   * Two things are never dropped. A name that does not fit the pattern is
   * somebody else's file and not ours to delete; and opts.keepNames is how a
   * restore protects the backup it is restoring FROM against the prune its
   * own save sets off.
   */
  var BACKUP_NAME = /^namelist-(\d{8})-(\d{6})\.xlsx$/i;

  function backupStamp(name) {
    var m = BACKUP_NAME.exec(name);
    return m ? m[1] + m[2] : '';
  }

  function backupsToKeep(names, opts) {
    names = names || [];
    opts = opts || {};
    var newest = opts.newest == null ? 3 : opts.newest;
    var days = opts.days == null ? 14 : opts.days;
    var keeping = {};
    var ours = [];

    names.forEach(function (name) {
      if (backupStamp(name)) ours.push(name);
      else keeping[name] = true;
    });
    (opts.keepNames || []).forEach(function (name) { keeping[name] = true; });

    ours.sort(function (a, b) {
      var x = backupStamp(a), y = backupStamp(b);
      if (x !== y) return x < y ? -1 : 1;
      return a < b ? -1 : a > b ? 1 : 0;
    });

    for (var i = ours.length - 1; i >= 0 && ours.length - i <= newest; i--) keeping[ours[i]] = true;

    var dates = [];
    var firstOn = {};
    ours.forEach(function (name) {
      var day = backupStamp(name).slice(0, 8);
      if (!firstOn[day]) { firstOn[day] = name; dates.push(day); }
    });
    dates.slice(Math.max(0, dates.length - days)).forEach(function (day) {
      keeping[firstOn[day]] = true;
    });

    return {
      keep: names.filter(function (n) { return keeping[n]; }),
      drop: names.filter(function (n) { return !keeping[n]; }),
    };
  }

  /* ---- exporting to a workbook ----------------------------------------
   * A namelist can be printed and read on screen; this is the third thing
   * anyone asks for — the same students in Excel, so they can be sorted,
   * marked up or mail-merged. The filtering belongs to the page; what lives
   * here is the part that has to be right or the file will not open at all.
   */
  ...
  var EXPORT_SPLITS = [
    { key: '', label: 'Everything on one sheet' },
    { key: 'class', label: 'Form class' },
    { key: 'level', label: 'Level' },
    { key: 'pg', label: 'Posting group' },
    { key: 'tg', label: 'TG / SG' },
    { key: 'group', label: 'Teaching group' },
    { key: 'teacher', label: 'Teacher' },
  ];

  /* Excel refuses to open a workbook at all if any sheet name breaks one of
   * these, and says nothing useful about which one did it:
   *   - 31 characters at most
   *   - none of \ / ? * [ ] :          ("SS/Hist" arrives here every time)
   *   - not blank, and not wrapped in apostrophes
   *   - unique, compared without case
   *   - not "History", which Excel reserves — and which splitting by subject
   *     or by teaching group produces on a humanities department's data
   * Truncating happens BEFORE the uniqueness suffix, or two long class names
   * both cut down to the same 31 characters and the file is rejected. */
  var SHEET_LIMIT = 31;
  var RESERVED_SHEET = { history: true };

  function exportSheetName(label, taken) {
    taken = taken || {};
    var name = norm(String(label == null ? '' : label))
      .replace(/[\\\/\?\*\[\]:]/g, ' ')
      .replace(/\s+/g, ' ')
      .replace(/^'+|'+$/g, '')
      .trim();
    if (!name) name = 'Sheet';
    if (RESERVED_SHEET[name.toLowerCase()]) name = name + ' list';
    name = name.slice(0, SHEET_LIMIT).trim() || 'Sheet';
    var base = name;
    var n = 2;
    while (taken[name.toLowerCase()]) {
      var tail = ' ' + n++;
      name = (base.slice(0, SHEET_LIMIT - tail.length).trim() + tail);
    }
    taken[name.toLowerCase()] = true;
    return name;
  }

  /* [{ label, students }] — every student in exactly one sheet, except under
   * "group" and "teacher", where somebody genuinely belongs to several and
   * appears on each. Anyone the split cannot place lands in a named leftover
   * sheet rather than being dropped. */
  function splitStudents(model, students, by) {
    var ix = null;
    var out = [];
    var seen = {};
    /* `group` is carried where a sheet IS a class, so the workbook can head
     * that sheet the way the namelist on screen heads it. */
    function bucket(label, student, group) {
      var k = normKey(label);
      if (!seen[k]) { seen[k] = { label: label, students: [], group: group || null }; out.push(seen[k]); }
      seen[k].students.push(student);
    }
    if (by === 'group' || by === 'teacher') {
      ix = buildIndexes(model);
      var wanted = {};
      students.forEach(function (s) { wanted[s.id] = true; });
      var loose = [];
      students.forEach(function (s) {
        if (!(ix.groupsByStudent.get(s.id) || []).length) loose.push(s);
      });
      (model.groups || []).forEach(function (g) {
        var members = (ix.membersByGroup.get(g.code) || []).filter(function (s) { return wanted[s.id]; });
        if (!members.length) return;
        if (by === 'group') {
          members.forEach(function (s) { bucket(g.name || g.code, s, g); });
        } else {
          var names = teacherNames(g);
          if (!names.length) names = ['(no teacher yet)'];
          names.forEach(function (t) {
            members.forEach(function (s) { bucket(t, s); });
          });
        }
      });
      if (loose.length) loose.forEach(function (s) { bucket('(in no class)', s); });
    } else if (!by) {
      out.push({ label: 'All students', students: students.slice() });
    } else {
      students.forEach(function (s) {
        var v = by === 'pg' ? normPg(s.pg) : by === 'tg' ? normTg(s.tg) : norm(s[by]);
        bucket(v || '(blank)', s);
      });
    }
    out.forEach(function (sheet) { sheet.students.sort(byClassThenName); });
    out.sort(function (a, b) { return cmp(a.label, b.label); });
    return out;
  }

  function exportCellValue(student, key) {
    if (key === 'sn') return norm(student.sn);
    if (key === 'tg') return normTg(student.tg);
    if (key === 'pg') return normPg(student.pg);
    if (key === 'level') return levelLabel(student.level);
    return norm(student[key]);
  }

  function exportHeader(columns) {
    return columns.map(function (key) {
      var col = EXPORT_COLUMNS.filter(function (c) { return c.key === key; })[0];
      return col ? col.label : key;
    });
  }

  /* ---- making the workbook look like a namelist -----------------------
   * SheetJS's free build writes values, column widths, merges and page
   * margins — and no cell formatting at all. No borders, no bold heading, no
   * frozen header row. A list people print and mark up by hand wants all
   * three, so the file is written, opened again as the zip it actually is,
   * its styles.xml replaced with one defining the few formats used, and each
   * cell tagged with the format its row wants. SheetJS's own CFB does the
   * unpacking and repacking, so nothing new is vendored and nothing runs
   * outside the page.
   *
   * Keep this minimal and in the order the schema demands — fonts, fills,
   * borders, cellStyleXfs, cellXfs, cellStyles — because Excel refuses a
   * whole workbook over a stylesheet it does not like, and says nothing
   * useful about why.
   */
  var XF = { plain: 0, cell: 1, head: 2, title: 3, quiet: 4 };
  ...
  var STYLES_XML = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
    ...
    '<fonts count="4">' +
      ...
      '<font><b/><sz val="15"/><color rgb="FF1F3864"/><name val="Calibri"/><family val="2"/><scheme val="minor"/></font>' +
      '<font><sz val="10"/><color rgb="FF6B7280"/><name val="Calibri"/><family val="2"/><scheme val="minor"/></font>' +
    '</fonts>' +
    '<fills count="3">' +
    ...
    '<borders count="2">' +
      ...
      '<border>' +
        '<left style="thin"><color rgb="FF9AA5B1"/></left>' +
        '<right style="thin"><color rgb="FF9AA5B1"/></right>' +
        '<top style="thin"><color rgb="FF9AA5B1"/></top>' +
        '<bottom style="thin"><color rgb="FF9AA5B1"/></bottom>' +
        '<diagonal/></border>' +
    '</borders>' +
  ...
  function utf8String(bytes) { return new TextDecoder().decode(new Uint8Array(bytes)); }

  /* plans[i] describes sheet i+1: `rows` gives the format for every cell of
   * each row, and `freezeAt` is the first row that should scroll. */
  function applyStyles(bytes, plans) {
    var CFB = XLSX.CFB;
    /* XLSX.write hands back an ArrayBuffer and CFB wants a plain array of
     * bytes, so neither end can be passed straight to the other. */
    var cfb = CFB.read(Array.prototype.slice.call(new Uint8Array(bytes)), { type: 'array' });
    CFB.utils.cfb_add(cfb, '/xl/styles.xml', utf8Bytes(STYLES_XML));
    plans.forEach(function (plan, i) {
      var path = '/xl/worksheets/sheet' + (i + 1) + '.xml';
      var entry = CFB.find(cfb, path);
      if (!entry) return;
      var xml = utf8String(entry.content);
      var rows = plan.rows || [];
      /* The cell reference carries its own row number, so no row parsing is
       * needed. A cell that already has a format is left as it is. */
      xml = xml.replace(/<c r="([A-Z]+)(\d+)"([^>]*)>/g, function (m, col, row, rest) {
        var sid = rows[+row - 1];
        if (!sid || / s="/.test(rest)) return m;
        return '<c r="' + col + row + '" s="' + sid + '"' + rest + '>';
      });
      if (plan.freezeAt > 1) {
        xml = xml.replace('<sheetView workbookViewId="0"/>',
          '<sheetView workbookViewId="0"><pane ySplit="' + (plan.freezeAt - 1) +
          '" topLeftCell="A' + plan.freezeAt + '" activePane="bottomLeft" state="frozen"/>' +
          '<selection pane="bottomLeft" activeCell="A' + plan.freezeAt +
          '" sqref="A' + plan.freezeAt + '"/></sheetView>');
      }
      CFB.utils.cfb_add(cfb, path, utf8Bytes(xml));
    });
    return new Uint8Array(CFB.write(cfb, { fileType: 'zip', type: 'array' }));
  }

  /* One blank column on either side and a blank row on top, so nothing is
   * jammed against the edge of the paper or the corner of the screen. */
  var EDGE = 1;
  var EDGE_WIDTH = 2.5;

  function columnWidths(header, body) {
    var wide = header.map(function (h) { return String(h).length; });
    body.forEach(function (row) {
      row.forEach(function (v, i) {
        var n = String(v == null ? '' : v).length;
        if (n > wide[i]) wide[i] = n;
      });
    });
    return [{ wch: EDGE_WIDTH }]
      .concat(wide.map(function (n) { return { wch: Math.max(7, Math.min(42, n + 2)) }; }))
      .concat([{ wch: EDGE_WIDTH }]);
  }

  /* A sheet, laid out the way the namelist on screen is laid out: what it is,
   * what it holds, then the roll. Returns the worksheet and the format of
   * every row, which applyStyles needs afterwards. */
  function exportSheet(sheet, columns, splitLabel) {
    var header = exportHeader(columns);
    var body = sheet.students.map(function (st) {
      return columns.map(function (key) { return exportCellValue(st, key); });
    });
    var meta = sheet.group ? namelistMeta(sheet.group, sheet.students) : null;
    var n = sheet.students.length;
    var aoa = [];
    var rows = [];
    var merges = [];
    var pad = function (row) { return [null].concat(row); };

    aoa.push([]); rows.push(XF.plain);                                  // air at the top
    aoa.push(pad([sheet.label])); rows.push(XF.title);
    aoa.push(pad([(meta && meta.subject ? meta.subject + ' · ' : splitLabel ? splitLabel + ' · ' : '') +
      n + ' student' + (n === 1 ? '' : 's')])); rows.push(XF.quiet);
    merges.push({ s: { r: 1, c: EDGE }, e: { r: 1, c: EDGE + Math.min(3, header.length - 1) } });
    merges.push({ s: { r: 2, c: EDGE }, e: { r: 2, c: EDGE + Math.min(3, header.length - 1) } });
    if (meta) {
      /* The same strip the namelist carries on screen: level, band, what the
       * class teaches, and the head count at the far end. With too few columns
       * to give each fact a cell of its own they share one, because losing the
       * head count off the end would be worse than a crowded cell. */
      var facts = [meta.level, meta.band, meta.subject].filter(Boolean);
      var tail = 'Total pax: ' + meta.total;
      var strip;
      if (header.length < 2) {
        strip = [facts.concat([tail]).join(' · ')];
      } else {
        strip = header.length > facts.length ? facts.slice() : [facts.join(' · ')];
        while (strip.length < header.length - 1) strip.push('');
        strip.push(tail);
      }
      aoa.push(pad(strip));
      rows.push(XF.cell);
    }
    aoa.push([]); rows.push(XF.plain);
    var headerRow = aoa.length + 1;                                     // 1-based, for the freeze
    aoa.push(pad(header)); rows.push(XF.head);
    body.forEach(function (row) { aoa.push(pad(row)); rows.push(XF.cell); });

    var ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!cols'] = columnWidths(header, body);
    ws['!rows'] = rows.map(function (sid, i) {
      return { hpt: i === 1 ? 22 : sid === XF.head ? 20 : 16 };
    });
    ws['!merges'] = merges;
    ws['!margins'] = { left: 0.5, right: 0.5, top: 0.7, bottom: 0.5, header: 0.3, footer: 0.3 };
    return { ws: ws, plan: { rows: rows, freezeAt: headerRow + 1 } };
  }

  /* The workbook: a Summary sheet first, then one per split. The summary is
   * there so somebody handed the file three weeks later can see what it is and,
   * more to the point, what it left out. */
  function exportWorkbook(model, sheets, opts) {
    opts = opts || {};
    var columns = opts.columns && opts.columns.length ? opts.columns : ['sn', 'class', 'name', 'gender'];
    var wb = XLSX.utils.book_new();
    var plans = [];
    var taken = {};
    var placed = sheets.map(function (sheet) {
      return { name: exportSheetName(sheet.label, taken), label: sheet.label,
        students: sheet.students, group: sheet.group || null };
    });
    if (opts.summary !== false) {
      var head = ['Sheet', 'Named for', 'Students'];
      var rows = [[], [null, 'Namelist export'], [null, 'Made', opts.madeAt || ''],
        [null, 'Split by', opts.splitLabel || ''], [null, 'Filters', opts.filterNote || 'none'],
        []];
      var styles = [XF.plain, XF.title, XF.quiet, XF.quiet, XF.quiet, XF.plain];
      rows.push([null].concat(head)); styles.push(XF.head);
      var body = [];
      placed.forEach(function (p) {
        body.push([p.name, p.label, p.students.length]);
        rows.push([null, p.name, p.label, p.students.length]);
        styles.push(XF.cell);
      });
      rows.push([]); styles.push(XF.plain);
      rows.push([null, 'Total rows', '', placed.reduce(function (n, p) { return n + p.students.length; }, 0)]);
      styles.push(XF.cell);
      var sws = XLSX.utils.aoa_to_sheet(rows);
      sws['!cols'] = columnWidths(head, body);
      sws['!merges'] = [{ s: { r: 1, c: EDGE }, e: { r: 1, c: EDGE + 2 } }];
      sws['!margins'] = { left: 0.5, right: 0.5, top: 0.7, bottom: 0.5, header: 0.3, footer: 0.3 };
      XLSX.utils.book_append_sheet(wb, sws, exportSheetName('Summary', taken));
      plans.push({ rows: styles, freezeAt: 0 });
    }
    placed.forEach(function (p) {
      var built = exportSheet(p, columns, opts.splitLabel);
      XLSX.utils.book_append_sheet(wb, built.ws, p.name);
      plans.push(built.plan);
    });
    return { wb: wb, plans: plans };
  }

  /* The file itself, ready to be handed to a download. */
  function exportBytes(model, sheets, opts) {
    var built = exportWorkbook(model, sheets, opts);
    var raw = XLSX.write(built.wb, { bookType: 'xlsx', type: 'array' });
    return applyStyles(raw, built.plans);
  }
  ...
  /* Two models compared by content rather than by how their keys happen to be
   * ordered. The editor uses this to ask "is what I hold now the same as what
   * is on disk?" — with a plain JSON.stringify that answer is false whenever
   * the two were built by different code paths, and the page then saves,
   * which makes the other editor save, for ever. */
  function modelSignature(model) {
    return JSON.stringify(model, function (k, v) {
      if (!v || typeof v !== 'object' || Array.isArray(v)) return v;
      return Object.keys(v).sort().reduce(function (o, key) {
        o[key] = v[key];
        return o;
      }, {});
    });
  ...
  function indexBy(list, key) {
    var out = {};
    (list || []).forEach(function (r) { if (!(r[key] in out)) out[r[key]] = r; });
    return out;
  }

  /* A workbook that has ever held two records under one id would otherwise
   * come out of the merge as two copies of the second one, with the first
   * silently gone. Keep the first of each id and say what was dropped. */
  function dedupeByKey(list, key, spec, out) {
    var seen = {};
    return (list || []).filter(function (r) {
      var k = r[key];
      if (seen[k]) {
        if (out && spec) {
          out.renamed.push('Two ' + spec.noun + ' records share ' + (k || '(no id)') +
            ' — kept "' + spec.label(seen[k]) + '", dropped "' + spec.label(r) + '"');
        }
        return false;
      }
      seen[k] = r;
   ...
   *
   * The comparisons are exact (on trimmed text), not case-folded: an admin who
   * spends an afternoon correcting capitalisation has made real changes, and
   * treating those as "unchanged" would let the other admin's next save revert
   * every one of them without so much as a conflict. norm() rather than raw
   * === so a missing field and an empty one are still the same thing. */
  function pick3(b, m, t) {
    if (norm(m) === norm(t)) return { value: m };
    if (norm(m) === norm(b)) return { value: t, from: 'theirs' };
    if (norm(t) === norm(b)) return { value: m, from: 'mine' };
  ...
  /* Field order is fixed rather than inherited from whichever side happened to
   * build the record — the caller compares a merged model against the file by
   * signature, and a record that differs only in key order would read as a
   * change and start the two editors saving at each other. */
  function mergeRecord(spec, b, m, t, out) {
    var rec = {};
    var label = spec.label(m) || spec.label(t);
    rec[spec.key] = m[spec.key];
    spec.fields.forEach(function (f) { rec[f] = ''; });
    (spec.sets || []).forEach(function (f) { rec[f] = []; });
    (spec.maps || []).forEach(function (f) { rec[f] = {}; });
    Object.keys(m).concat(Object.keys(t)).sort().forEach(function (k) {
      if (k in rec) return;
      rec[k] = (k in m) ? m[k] : t[k];
    });
  ...
  function mergeCollection(spec, base, mine, theirs, out) {
    var baseList = dedupeByKey(base[spec.name], spec.key);
    var mineList = dedupeByKey(mine[spec.name], spec.key, spec, out);
    var theirsList = dedupeByKey(theirs[spec.name], spec.key, spec, out);
    var b = indexBy(baseList, spec.key);
    var m = indexBy(mineList, spec.key);
    var t = indexBy(theirsList, spec.key);
    var order = [];
    mineList.forEach(function (r) {
      if (order.indexOf(r[spec.key]) === -1) order.push(r[spec.key]);
    });
    theirsList.forEach(function (r) {
      if (order.indexOf(r[spec.key]) === -1) order.push(r[spec.key]);
    });
    var res = [];
  ...
  /* Subject columns are matched case-insensitively, but a student's subjects
   * are held in a plain object keyed by the exact spelling. Left alone, "Hist"
   * from one admin and "HIST" from the other survive as two keys while only
   * one is listed in subjectKeys — and modelToWorkbook writes only the listed
   * columns, so the other one's values are dropped at the next save. Rewrite
   * every side onto one spelling before anything is compared. */
  function alignSubjectKeys(keys, sides) {
    var canonical = {};
    keys.forEach(function (k) { canonical[normKey(k)] = k; });
    sides.forEach(function (side) {
      (side.students || []).forEach(function (s) {
        if (!s.subjects) return;
        var next = {};
        Object.keys(s.subjects).forEach(function (k) {
          var to = canonical[normKey(k)] || k;
          if (!(to in next) && norm(s.subjects[k])) next[to] = s.subjects[k];
        });
        s.subjects = next;
      });
    });
  ...
  function mergeModels(base, mine, theirs) {
    ...
    var subjectKeys = mergeSet(base.subjectKeys, mine.subjectKeys, theirs.subjectKeys);
    alignSubjectKeys(subjectKeys, [base, mine, theirs]);
  ...
  function mergeRequests(model, items) {
    ...
    var added = 0, repeat = 0, decided = 0, reopened = 0;
    items.forEach(function (it) {
      ...
      /* Asked again after being turned down. A suggestion keeps the same id
       * however many times it is sent — that is what stops one email arriving
       * twice from becoming two cards — so the only way to tell "sent again"
       * from "asked again" is when it was sent: after the refusal means the
       * teacher is making the case a second time, and it deserves an answer.
       * Something already DONE is not reopened; the work exists. */
      if (normKey(have.status) === 'dismissed' && it.made && have.decided &&
        it.made > have.decided) {
        have.status = 'open';
        have.made = it.made;
        have.reason = it.reason || have.reason;
        have.decided = '';
        reopened++;
        return;
      }
      if (have.status && have.status !== 'open') decided++;
      else repeat++;
    });
    return { added: added, repeat: repeat, decided: decided, reopened: reopened };
  }
