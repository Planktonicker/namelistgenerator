...
(function () {
  ...
  function exportCellValue(student, key, subjectKeys) {
    if (key.indexOf('s:') === 0) return norm(student.subjects && student.subjects[key.slice(2)]);
  ...
  function exportHeader(columns, subjectKeys) {
    return columns.map(function (key) {
      if (key.indexOf('s:') === 0) return key.slice(2);
  ...
  function exportWorkbook(model, sheets, opts) {
    ...
    var subjectKeys = model.subjectKeys || [];
    ...
    placed.forEach(function (p) {
      var aoa = [exportHeader(columns, subjectKeys)];
      p.students.forEach(function (s) {
        aoa.push(columns.map(function (key) { return exportCellValue(s, key, subjectKeys); }));
      });
